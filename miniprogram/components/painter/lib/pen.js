...
export default class Painter {
  ...
  _border({ borderRadius = 0, width, height, borderWidth = 0, borderStyle = 'solid' }) {
    ...
    if (borderRadius) {
      ...
      if (border.length === 4) {
        ...
        r1 =
          ...
          r3 =
          r4 =
            Math.min(borderRadius && borderRadius.toPx(false, minSize), width / 2, height / 2)
      }
    }
    ...
    r2 !== 0 &&
      this.ctx.arc(width / 2 - r2, -height / 2 + r2, r2 + lineWidth / 2, 1.5 * Math.PI, 2 * Math.PI) // 右上角圆弧
    this.ctx.lineTo(
      width / 2 + lineWidth / 2,
    ...
    r4 !== 0 &&
      this.ctx.arc(-width / 2 + r4, height / 2 - r4, r4 + lineWidth / 2, 0.5 * Math.PI, 1 * Math.PI) // 左下角圆弧
    this.ctx.lineTo(
      -width / 2 - lineWidth / 2,
  ...
  _preProcess(view, notClip) {
    ...
    const align =
      view.css && view.css.align ? view.css.align : view.css && view.css.right ? 'right' : 'left'
