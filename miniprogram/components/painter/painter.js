  ...
  methods: {
    ...
    getScaleIcon(rect, type) {
      ...
      if (customActionStyle && customActionStyle.scale) {
        scaleArea = {
          type: 'image',
          url:
            type === 'text' ? customActionStyle.scale.textIcon : customActionStyle.scale.imageIcon,
          css: {
    ...
    isInDelete(x, y) {
      for (const view of this.block.views) {
        if (view.id === 'delete') {
          return (
            x > view.rect.left && y > view.rect.top && x < view.rect.right && y < view.rect.bottom
          )
        }
      }
      return false
    ...
    initDancePalette() {
      ...
      this.downloadImages(this.properties.dancePalette).then(async palette => {
        ...
        this.frontContext ||
          (this.frontContext = await this.getCanvasContext(this.properties.use2D, 'front'))
        this.bottomContext ||
          (this.bottomContext = await this.getCanvasContext(this.properties.use2D, 'bottom'))
        this.topContext ||
          (this.topContext = await this.getCanvasContext(this.properties.use2D, 'top'))
        this.globalContext ||
          (this.globalContext = await this.getCanvasContext(this.properties.use2D, 'k-canvas'))
        ...
        wx.canvasToTempFilePath(
          {
            ...
            fail: function (error) {
              ...
              wx.getFileSystemManager().writeFile({
                ...
                encoding: 'utf8',
                success: res => {
                  that.getImageInfo(filePath)
                },
                fail: err => {
                  console.error(`getFileSystemManager failed, ${JSON.stringify(err)}`)
