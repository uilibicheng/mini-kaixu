 ...
 */
export const dateFormat: any = (date: any, format: string) => {
  // 秒需要转成毫秒
  if (/^\d{10}$/.test(date)) {
    date = date * 1000
  }
...
export const isSpecialChar = (str: string) => {
  const specialChar = ['=', '.', '@']
...
// url 参数拼接
export const queryString = (data: any, isEncode = false) => {
  const str = []
  for (const k in data) {
    let v = data[k]
...
export function throttle(func: Function, wait = 200) {
  let previous = 0 // 初始化previous为0

  return function (this: object, ...args: any) {
    let now = Date.now()
    let context = this

    // 只有当当前时间与上一次执行的时间差大于wait时，才执行函数
    if (now - previous >= wait) {
      func.apply(context, args)
      previous = now // 更新previous为当前时间
    }
  }
}

export function formatHttps(link: string) {
